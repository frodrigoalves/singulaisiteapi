...
export function WhatIsSingulaiSection() {
  return (
    <Section id="singulai-pen" spacing="xl" className="relative overflow-hidden px-4 md:px-0">
      {/* Background accent */}
      <div className="absolute top-1/2 left-0 w-64 md:w-96 h-64 md:h-96 bg-primary/10 rounded-full blur-[128px] -translate-y-1/2" />
      ...
      <div className="grid lg:grid-cols-2 gap-8 lg:gap-20 items-center">
        ...
        <div className="relative order-2 lg:order-1">
          ...
          {/* Floating badge */}
          <div className="absolute -bottom-2 right-2 md:-bottom-4 lg:bottom-8 md:-right-4 lg:-right-8 animate-float">
            <GlassCard size="sm" className="flex items-center gap-2 p-2 md:p-3">
              <div className="w-2 h-2 md:w-3 md:h-3 rounded-full bg-green-500 animate-pulse" />
              <span className="text-[10px] md:text-sm font-medium text-foreground">LGPD/GDPR Compliant - Ethereum Secured</span>
            </GlassCard>
          </div>
        </div>
        ...
        <div className="space-y-6 md:space-y-8 order-1 lg:order-2">
          <div className="space-y-3 md:space-y-4">
            <h2 className="text-2xl md:text-h2 font-bold text-foreground">
            ...
            <p className="text-xs md:text-body text-muted-foreground leading-relaxed">
          ...
          <div className="grid grid-cols-2 gap-3 md:gap-4">
            {features.map((feature) => (
              <div
                key={feature.title}
                className="flex flex-col gap-2 p-3 md:p-4 rounded-lg md:rounded-xl bg-secondary/50 border border-border"
              >
